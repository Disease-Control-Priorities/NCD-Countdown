...
all.locs      <- countries
increases     <- seq(0, 0.05, 0.01)
covidscens    <- seq(0, 2, 0.25)
...
for (is in all.locs){
  ...
  for (sinc in increases){
    ...
    setWinProgressBar(pb, k, title=paste0(round(k/total*100,3),"% done"))
      ...
      proj  <- project_pop(is, inter, sinc, "yes", sel.cse, "varying", "no", "yes", sc.covid)
...
# Make diff deaths NCD specific
# x axis being covid scalar running from 0 to 2
# change rate of increase to absolute p.p number instead of relative  (0% to 5% per year)

goal.q30.out   <- rbindlist(goal.q30)
ncd.deaths.out <- rbindlist(ncd.deaths)
all.q30.out <- rbindlist(q30.df) %>%
...
save(goal.q30.out, ncd.deaths.out, summary, all.q30.out, adj.D.P, q30.sum, 
     file = "output/goal.q30.covid.Rda")
