...
load("../new_demography/output/icer_rank_output_0819.Rda")
...
#cpi<-read.csv("../new_inputs/CPI_2020.csv", stringsAsFactors=F)
#names(cpi)[1]<-"Country"

# GNI
GNI <- read.csv("../new_inputs/GNI_WB_USD_2017_re2.csv", stringsAsFactors=F)
#GNI <- read.csv("../new_inputs/GNI_2020.csv", stringsAsFactors=F)
#names(GNI)[1]<-"Country"
...
# All data saved
```{r}
file.name <- paste0("../new_demography/output/costs/All_total_cost_2030_intervention_0819",".csv")
write.csv(final_all_intervention, file.name)

file.name <- paste0("../new_demography/output/costs/All_total_cost_2030_year_0819",".csv")
write.csv(final_all, file.name)

file.name <- paste0("../new_demography/output/All_ICER_2030_0819",".csv") #FOR WILL's CODE
write.csv(final_all_ICER, file.name)

file.name <- paste0("../new_demography/output/costs/All_increment_2030_0819",".csv")
...
write.csv(df.uc.class_wide, "../new_demography/output/costs/Mean_UC_0819.csv")
...
write.csv(base_pin_df_wide, "../new_demography/output/costs/Baseline_pop_0819.csv")
...
write.csv(adjust_pin_df_wide, "../new_demography/output/costs/Adjusted_pop_0819.csv")
...
write.csv(increment_df_wide, "../new_demography/output/costs/Incremental_cost_0819.csv")
...
# Check output
```{r}
icer <- read.csv("../new_demography/output/All_ICER_2030_0819.csv" , stringsAsFactors=F)
...
file.name <- paste0("../new_demography/output/costs/All_increment_ly_sum_0819",".csv")
...
file.name <- paste0("../new_demography/output/costs/All_increment_ly_sum_re_0819",".csv")
write.csv(icer.sum, file.name)
...
write.csv(ly_wide, "../new_demography/output/costs/ICER_region_NCD_0819.csv") #ICER by DALYs
write.csv(ly_wide2, "../new_demography/output/costs/ICER_region_NCD2_0819.csv") #ICER by 40q30
...
write.csv(rank_wide, "../new_demography/output/costs/Rank_region_NCD_0819.csv") #Ranked by DALYs
write.csv(rank_wide2, "../new_demography/output/costs/Rank_region_NCD2_0819.csv") #Ranked by 40q30
...
# Save
write.csv(percent_wide2, "../new_demography/output/costs/Percent_region_NCD_0819.csv")
...
#rank by 40q30
#rank<-rank_wide2
rank<-read.csv("../new_demography/output/costs/Rank_region_NCD2_0819.csv")%>%select(-c(X)) 
#rank_daly<-rank_wide
rank_daly<-read.csv("../new_demography/output/costs/Rank_region_NCD_0819.csv")%>%select(-c(X)) 
#color code by DALYs
#icer<-percent_wide2
icer<-read.csv("../new_demography/output/costs/Percent_region_NCD_0819.csv")%>%select(-c(X)) 
#compare the two icer metrics for double legend
#icer2<-ly_wide2
icer2<-read.csv("../new_demography/output/costs/ICER_region_NCD2_0819.csv")%>%select(-c(X)) 
...
ggplot(dt%>%group_by(Code)%>%summarise(icer_40q30=mean(icer_40q30), icer_gdp = mean(icer_gdp),
  ...
  xlab("Log(ICER as a share of GDP per capita)")+
  ylab("Log(% change in 40q30 \nper $1 million)")+
  geom_label_repel(aes(label=text),box.padding = 0.1, point.padding=0.1)

ggsave("Figures/AdditionalFigure_option3_0819.png", height = 7, width = 11, units = "in")
...
#my_breaks<-c(0.1,0.3,0.5,1,1.5,2)
  ...
  ggsave("Figures/Figure1_0819.png", height = 7, width = 9, units = "in")
...
caseload<-cases%>%group_by(Code)%>%summarise(volume_per1e5pop=sum(cases/pop)*100000)
...
write.csv(table4, "Figures/Table4_0819.csv")
...
#add deaths??
#add intersectoral interventions for dalys, 40q30
...
write.csv(tableA2, "../new_demography/Figures/TableA2_0819.csv")
