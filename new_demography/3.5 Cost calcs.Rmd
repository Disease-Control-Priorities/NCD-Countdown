...
```{r}
file.name <- paste0("output/Optimal/All_TC_2030_0819",".csv")
write.csv(final_all, file.name)

file.name <- paste0("output/Optimal/All_Intervention_2030_0819",".csv")
write.csv(final_all_intervention, file.name)

file.name <- paste0("output/Optimal/All_ICER_2030_0819",".csv")
...
#load("output/Optim.Out_0819.Rda")
file.name <- paste0("output/Optimal/All_Intervention_2030_0819",".csv")
all <- read.csv(file.name)

##add intersectoral costs##
int_cost<-read.csv("output/costs/All_total_cost_2030_intervention_0819.csv", stringsAsFactors = F)%>%select(-c(X))%>%
...
table3[,2:4] <- round(table3[,2:4] / 7,2) #annual
...
dth_avert<-left_join(dadt.all.opt, regions%>%rename(location_name=Country), by="location_name")%>%group_by(NCD_region)%>%summarise(DA = sum(Deaths.Avert))
...
table3<-left_join(table3%>%select(-c(Nx)), vsl%>%select(c(Region, DA, net.econ.pc)), by="Region")
table3$DA<-table3$DA/1000
#table3[,5:7]<-round(table3[,5:7])

#add benefit cost ratio
table3$benefit.cost.ratio<-round(table3$net.econ.pc/(table3$increment_pc*7),2)

write.csv(table3, "Figures/Table3_0819.csv")

vsl<-vsl%>%select(c(Region, increment, DA, VSL, econ.bns, net.econ.pc))
vsl$DA<-vsl$DA/1000
vsl$VSL<-vsl$VSL/1000

write.csv(vsl, "Figures/TableA8.csv")
```
...
save(q30.summary1, da, file = "output/Optim.Out.Tables0819.Rda")

write.csv(q30.summary1, "Figures/Table2_0819.csv")
```
...
write.csv(sum_country2, "Figures/Total_cost_NCD_forAdrian0819.csv")
```

# Appendix Table A3
```{r additional table}

rank<-read.csv("output/All_ICER_2030_0819.csv", stringsAsFactors = F)
rank$included<-NA
...
#View(tableA3)
write.csv(tableA3, "../new_demography/Figures/TableA3.csv")
...
write.csv(ta, "../new_demography/Figures/TableA6.csv")

```

#Appendix Table A9
```{r}
tc<-read.csv("../new_demography/output/Optimal/All_Intervention_2030_0819.csv")
