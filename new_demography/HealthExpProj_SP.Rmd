---
title: "Health expenditure projections in LMICs 2020-2030\nPRELIMINARY results"
...
#remove data
rm(list= ls()[!(ls() %in% c('weo_raw','gdp_growth_raw', 'wpp_raw', 'wpp_age_raw', 'cc', 'ghed_raw','ihme_raw', 'ncd_costs_raw', 'ncd_costs_latest_raw', 'ncd_costs_july_ncd4'))])

### UPDATE DATA ###
ncd_costs_aug <- read.csv("Figures/Total_cost_NCD_forAdrian0803.csv") 
...
USD2017_defl<-weo %>%
  ...
  mutate(GDPdefl_ratio=value[which(year==2017)]/value) %>%
...
core_df1<-ncd_costs_new %>%
  ...
  left_join(ncd_costs_new %>% select(ISO, Class, Region)) %>%
...
gdp_limits<-data.frame(year=seq(2023,2030), #start @ 2023,2030
                       gdp_ceiling=c(rep(1.06,3), rep(1.05,4))) #might update this window as well
...
ggsave("GGHE_results/GDPmethod.png")
...
ggsave("GGHE_results/GDPmethod2.png")
...
ggsave("GGHE_results/GDPprojbyregion_vs_IHME.png")
...
ggsave("GGHE_results/GDPprojbyregion.png")
```
...
gghed_limits<-gghed %>%
  ...
  summarise(Average=mean(GGHED_GDP),
            ...
            IQR=IQR(GGHED_GDP),
            p80=quantile(GGHED_GDP, 0.80),
            p20=quantile(GGHED_GDP, 0.20)) %>%
  arrange(Region) 
...
main_df_gghe0%>%
  ...
  geom_point() +
  facet_wrap(~Class, scales="free") + #income_group was maybe renamed Class? #SP
  theme(legend.position = "bottom")
```
...
ggsave("GGHE_results/GGHED_GDPbyregion_vs_IHME.png", width=10, height=8)
...
ggsave("GGHE_results/GGHED_GDPbyregion.png", width=10, height=8)
...
plot1 <- ncd_gghed_boxplots2a %>% 
  filter(year %in% c(2023, 2026, 2030), GGHEDscenario=="own projections",
...
write.table(table_income, "GGHE_results/table_income.txt", sep=",", row.names = FALSE)
write.table(table_region, "GGHE_results/table_region.txt", sep=",", row.names = FALSE)
