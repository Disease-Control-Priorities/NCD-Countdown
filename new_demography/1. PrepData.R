...
country.lab <- fread("../new_inputs/Country_groupings.csv") %>%
  filter(!is.na(NCD_region) & World_bank %!in% "HIC" & Country != "")  %>%
...
efficacy      <- readxl::read_excel("../new_inputs/EfficacyData0625.xlsx") %>%
  rename(Code=NCD, agel = `start age`, ageu = `stop age`) %>%
  mutate(cause_name= ifelse(Level4 != "NA", `Level4`, `Level3`),
         ageu = ifelse(ageu=="95+","85",ageu), ageu = as.numeric(ageu), agel = as.numeric(agel))

ncd4          <- efficacy %>% select(Code, NCD4, calc_ICER) %>% distinct()

efficacy2     <- efficacy %>%
  select(Intervention, Code,  sub_id, cause_name, `Mortality reduction`, agel, ageu) %>%
  filter(!is.na(ageu)) %>%
...
pin.groups <- fread("../new_inputs/PINandCosts0625.csv") %>% 
...
yldyll <- fread("../new_inputs/YLDtoYLLratio.csv") %>% 
  rename(location_name = location, cause_name = cause) %>% 
  mutate(scale = YLD/YLL, 
         iso3 = countrycode::countrycode(location_name, "country.name", "iso3c")) %>% 
  select(iso3, cause_name, scale) %>%
  filter(cause_name %in% c("All Causes", cse_g$cause_name)) %>% 
  right_join(country.lab, by = "iso3")
...
drops <- c("ghe.in", "wpp.input", "efficacy", "efficacy2", "ta_delay", 
           "loc", "locs","coverage", "all.mx", "obs_wpp")
rm(list = c(drops,"drops"))

##############################################################################################################

save.image(file = "../new_inputs/PreppedData0819.Rda")
