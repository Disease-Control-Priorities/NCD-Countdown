...
#remove data
rm(list= ls()[!(ls() %in% c('weo_raw','gdp_growth_raw', 'wpp_raw', 'wpp_age_raw', 'cc', 'ghed_raw',
...
#GDP deflator rebased to 2017
USD2017_defl<-weo %>%
  filter(indicator=="Gross domestic product, deflatorIndex", year>=2000, ISO=="USA") %>%
  select(year, value) %>%
  #use 2% yearly growth to project GDP deflator over 2025-2030
  rbind(data.frame(year=seq(2025,2030), value=rep(0,6))) %>%
  mutate(value=ifelse(year>2024, value[which(year==2024)]*1.02^(year-2024), value)) %>%
  mutate(GDPdefl_ratio=value[which(year==2017)]/value) %>% #to update to 2018 USD here
...
bind_rows(dx0, dx1) %>%
  ...
  scale_colour_manual(name="dataset and scenario",
                      ...
                      labels = c("December 2019 data, blue scenario", "December 2019 data, green scenario",
                                 "June 2020 data")) +
...
gdp_proj_test %>%
  filter(year>2021, year<=2024, !is.na(GDP_USD2017_off)) %>%
...
proj_gdp <- expand.grid(year_startpred=2025, years_back=c(3,4,5), 
                        mean_type=c("arith", "geom"), window_type=c("fixed", "rolling"))
...
gdp_proj_all1 %>%
  ...
  filter(is.na(GDP_USD2017), year>2024)
```
...
ggsave("GGHE_results/original_results/GDPprojbyregion.png")
```
...
#test and check
test01<-fun_gghed_fwdproj(main_df1, year_startpred=2018, years_back=4, 
                          mean_type="arith", window_type="rolling")
...
arima_iso_test <- fun_gghed_arima_test(gghed, "ROU", 2015, 2017)
...
fun_gghed_arima_pred <- function(df, x, y1, y2){
  ...
  df1 <- df %>%
    ...
    filter(grepl("ARIMA", GGHEDmethod), ISO==x, year<2018) %>%
...
gghed_proj_test_reg <- fun_gghed_regproj(2015, 2017, regmodel_list)
...
gghed_fwdproj1<-fun_gghed_fwdproj(main_df1, year_startpred=2018,
                   years_back=5, mean_type="geom", window_type="fixed")
...
main_df_gghe0 %>%
  ...
  geom_line() +
  labs(y="GGHED % GDP", x="year",
       caption = "Note: Projections start after the dotted line (2018 onwards).") +
  scale_x_continuous(breaks=c(2010, 2015, 2020, 2025, 2030)) +
  theme_minimal() +
  geom_vline(xintercept = 2018, linetype="dotted", color = "blue", size=0.5) +
  guides(colour = guide_legend(ncol = 2, bycol=TRUE)) +
  theme(legend.position="bottom")
ggsave("GGHE_results/original_results/GGHED_GDPbyregion.png", width=10, height=8)
...
ncd_gghed_boxplots1<-main_df_gghe_work1 %>%
  left_join(ncd_costs1 %>% select(ISO, year, ncd_costs_2017, total_cost_all)) %>%
...
ggsave("GGHE_results/original_results/GGHED_NCD_all_region_boxplot.png", width = 10, height = 7, units = "in")
...
ggplot(data=ncd_gghed_boxplots1 %>% filter(year==2030), aes(NCD_GGHED_all_old, Region)) +
  ...
  geom_text(data=ncd_gghed_boxplots1 %>% filter(NCD_GGHED_all_old>200, year==2030), 
            aes(label=ISO), nudge_x=0.2, nudge_y=0.2, size=2)
ggsave("GGHE_results/original_results/GGHED_NCD_all_OLD_region_boxplot.png", width = 10, height = 7, units = "in")
...
ggsave("GGHE_results/original_results/GGHED_NCD_all_region_boxplot_IHME.png", width = 7, height = 12, units = "in")
```
...
#bar chart
ncd_gghed_boxplots1 %>% 
  filter(year %in% c(2020, 2025, 2030), GGHEDscenario=="own projections") %>%
...
ggsave("GGHE_results/original_results/GGHED_NCD_region_bar.png", width = 12, height = 7, units = "in")
...
plot1 <- ncd_gghed_boxplots2a %>% 
  ...
  theme(legend.position = "none",
        axis.text.y = element_blank(),
        strip.text.y.left=element_text(angle = 0))
ggsave("GGHE_results/original_results/GGHED_NCD_region_bar_ncd4_baseline.png", width = 12, height = 7, units = "in")
...
plot2 <- ncd_gghed_boxplots2a %>% 
  ...
  theme(legend.position = "none",
        axis.text.y = element_blank(),
        strip.text.y.left=element_text(angle = 0))
ggsave("GGHE_results/original_results/GGHED_NCD_region_bar_ncd4_adjusted.tiff", width = 12, height = 7, units = "in", dpi = 300)

plot3 <- ncd_gghed_boxplots2b %>% 
  filter(year %in% c(2020, 2025, 2030), GGHEDscenario=="own projections",
         scenario=="Baseline", ISO!="VEN") %>%
  select(ISO, year, Region, NCD_GGHED) %>%
  group_by(year, Region) %>%
  ...
  facet_grid(str_wrap(Region, 25)~year, scales = "free", switch = "y",
             labeller = labeller(facet_category = label_wrap_gen(width = 20))) +
  labs(x="Region", y="Total self-harm costs as a share of total GGHE-D (%)",
       title = "Baseline scenario") +
  coord_flip() +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.y = element_blank(),
        strip.text.y.left=element_text(angle = 0))
ggsave("GGHE_results/original_results/GGHED_NCD_region_bar_self_baseline.png", width = 12, height = 7, units = "in")

plot4 <- ncd_gghed_boxplots2b %>% 
  filter(year %in% c(2020, 2025, 2030), GGHEDscenario=="own projections",
         scenario=="Adjusted", ISO!="VEN") %>%
  select(ISO, year, Region, NCD_GGHED) %>%
  group_by(year, Region) %>%
  ...
  facet_grid(str_wrap(Region, 25)~year, scales = "free", switch = "y",
             labeller = labeller(facet_category = label_wrap_gen(width = 20))) +
  labs(x="Region", y="Total self-harm costs as a share of total GGHE-D (%)",
       title = "Adjusted scenario") +
  coord_flip() +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.y = element_blank(),
        strip.text.y.left=element_text(angle = 0))
ggsave("GGHE_results/original_results/GGHED_NCD_region_bar_self_adjusted.png", width = 12, height = 7, units = "in")

plot12 <- grid.arrange(plot1, plot2, ncol = 1)
ggsave("GGHE_results/original_results/GGHED_NCD_region_bar_ncd4_combined.png", plot12, width = 12, height = 14, units = "in")

plot34 <- grid.arrange(plot3, plot4, ncol = 1)
ggsave("GGHE_results/original_results/GGHED_NCD_region_bar_self_combined.png", plot12, width = 12, height = 14, units = "in")
