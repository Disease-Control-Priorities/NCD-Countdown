...
project_pop <- function(is, inter, inc.val, tas, sel.cse, inc.type, est.pin, covid.in, sc.covid= NULL){
  ...
  in.df.in    <-  int.df %>% 
    ...
    mutate(inc = ifelse(Code %in% inter, inc.val, 0),
           `Baseline Coverage`=as.numeric(`Baseline Coverage`)) %>%
  ...
  doproj <- function(s.dp){
    ...
    for(i in 1:n){
      ...
      tax.df2 <- tax.df2 %>% mutate(Impact = 0.8*ifelse(s.dp == 0, 0, Impact))
      ...
      salt.df2 <- salt.df2 %>% mutate(Impact = 0.8*ifelse(i < 5, 0, s.dp*(1 - Impact)))
      ...
      tf.df2 <- tf.df2 %>% mutate(Impact = 0.8*ifelse(i < 5, 0, s.dp*(1 - Impact)))
      ########################################################################################################
      ...
      in.dfb             <- in.df.in %>%
        mutate(ic = ifelse(i < 5, 0, i - 4),
               ...
               `Impact` = 0.8*((`Effect (I)`*(`Target (P1)`-`Baseline Coverage`))/
